...
if __name__ == '__main__':
    ...
    # KFold Method
    kf = sklearn.model_selection.KFold(n_splits=k_times, random_state=seed, shuffle=True)
    kf.get_n_splits(X)

    # KFold Cross Validation
    counter = 0
    train_join_predictions = []
    test_join_predictions = []
    train_join_reals = []
    test_join_reals = []

    for train_index, test_index in kf.split(X):
        X_train, X_test = X[train_index], X[test_index]
        Y_train, Y_test = original_labels[train_index], original_labels[test_index]
        ...
        plt.title('model accuracy iterazione ' + str(counter))
        plt.ylabel('accuracy')
        plt.xlabel('epoch')
        ...
        plt.title('model loss iterazione ' + str(counter))
        plt.ylabel('loss')
        plt.xlabel('epoch')
        plt.legend(['train', 'test'], loc='upper left')
        plt.show()
        ...
        # predizioni e reali, e poi unisco al vettore principale

        train_join_predictions = train_join_predictions + [int(round(x[0])) for x in model.predict(X_train)]
        test_join_predictions = test_join_predictions + [int(round(x[0])) for x in model.predict(X_test)]

        train_join_reals = train_join_reals + [Y_train[i] for i in range(Y_train.shape[0])]
        test_join_reals = test_join_reals + [Y_test[i] for i in range(Y_test.shape[0])]

    # # METRICS
    #
    # print('Lunghezza finale etichette train:')
    # print(len(train_join_predictions))
    # print(len(train_join_reals))
    # print('Lunghezza finale etichette test:')
    # print(len(test_join_predictions))
    # print(len(test_join_reals))
    #
    # print('Model scores (K-fold Method):')
    #
    # print('Confusion Matrix (train set):')
    # print(sklearn.metrics.confusion_matrix(y_true=train_join_reals, y_pred=train_join_predictions, labels=np.unique(train_join_reals)))
    # print('Confusion Matrix (test set):')
    # print(sklearn.metrics.confusion_matrix(y_true=test_join_reals, y_pred=test_join_predictions, labels=np.unique(test_join_reals)))
    #
    # print("Accuracy (train set):",
    #       sklearn.metrics.accuracy_score(y_true=train_join_reals, y_pred=train_join_predictions))
    # print("Accuracy (test set):",
    #       sklearn.metrics.accuracy_score(y_true=test_join_reals, y_pred=test_join_predictions))
    #
    # print('Classification results (train set):')
    # print(sklearn.metrics.classification_report(y_true=train_join_reals, y_pred=train_join_predictions))
    # print('Classification results (test set):')
    # print(sklearn.metrics.classification_report(y_true=test_join_reals, y_pred=test_join_predictions))
    #
    # # Confusion Matrix Plot Train
    # matrix = sklearn.metrics.confusion_matrix(y_true=train_join_reals, y_pred=train_join_predictions, normalize='all')
    ...
    # matrix = sklearn.metrics.confusion_matrix(y_true=test_join_reals, y_pred=test_join_predictions, normalize='all')
    ...
    print('Confusion Matrix (train set):')
    print(sklearn.metrics.confusion_matrix(y_true=train_join_reals, y_pred=train_join_predictions,
                                           labels=np.unique(train_join_reals)))
    print('Confusion Matrix (test set):')
    print(sklearn.metrics.confusion_matrix(y_true=test_join_reals, y_pred=test_join_predictions,
                                           labels=np.unique(test_join_reals)))
    ...
    plt.title('Model Classifier Confusion Matrix\nK-fold Cross-Validation, Train Set')
    plt.show()
    plt.clf()  # clear plot for next method
    ...
    plt.title('Model Classifier Confusion Matrix\nK-fold Cross-Validation, Test Set')
    plt.show()
    plt.clf()  # clear plot for next method
